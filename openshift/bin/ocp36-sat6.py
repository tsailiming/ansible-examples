...
class ocpSat6(object):
    ...
    def __init__(self, load=True):
        ...
        if load:
            self._parseCli()
            self._loadImageList(['openshift3'. 'rhgs3'])
            self._addData()
            self._syncData()

    def _loadImageList(self, images):

        for image in images:
            cmd='curl -s https://registry.access.redhat.com/v1/search?q="' + image + '" | python -mjson.tool | grep ".name.:" | cut -d: -f2 | sed -e "s/ "//g"" -e "s/,"//g""'
